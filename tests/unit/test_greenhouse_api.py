...
class TestGreenhouseClient:
    ...
    def test_make_request_success(self, client, mock_urlopen):
        ...
        # Mock response
        mock_response = Mock()
        mock_response.read.return_value = json.dumps({"id": 123, "name": "Test"}).encode()
    ...
    def test_get_jobs_with_applications_single_page(self, mock_print, client):
        ...
        # Mock responses
        jobs_response = [
            {"id": 1, "name": "Job 1", "status": "open", "departments": [{"name": "Eng"}], "created_at": "2024-01-01"},
            {"id": 2, "name": "Job 2", "status": "closed", "departments": [], "created_at": "2024-01-02"}
        ]
    ...
    def test_get_jobs_with_applications_pagination(self, client):
        ...
        jobs_page1 = [{"id": i, "name": f"Job {i}", "status": "open", 
                      "departments": [{"name": "Dept"}], "created_at": "2024-01-01"}
                     for i in range(1, 101)]
        jobs_page2 = [{"id": i, "name": f"Job {i}", "status": "open",
                      "departments": [{"name": "Dept"}], "created_at": "2024-01-01"} 
    ...
    def test_get_applications_single_page(self, client, mock_urlopen):
        ...
        apps_data = [
            {"id": 1, "candidate_id": 100},
            {"id": 2, "candidate_id": 101}
        ]
        ...
        mock_urlopen.side_effect = [
            mock_response,
            Mock(__enter__=lambda self: Mock(read=lambda: b"[]"), __exit__=lambda self, *args: None)
        ]
    ...
    def test_get_jobs_missing_department_data(self, client):
        ...
        jobs_response = [
            ...
            {"id": 2, "name": "Job 2", "status": "open", "departments": [], "created_at": "2024-01-02"},
            {"id": 3, "name": "Job 3", "status": "open", "departments": [{"name": "HR"}], "created_at": "2024-01-03"},
        ]
