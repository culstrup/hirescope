...
"""
    ...
    def test_score_candidate_success(self, mock_urlopen, scorer, sample_candidate_profile, sample_job_description):
            ...
            "choices": [{
                "message": {
                    "content": json.dumps({
                        ...
                        "key_strengths": ["Python expertise", "AWS experience", "Leadership"],
                        "concerns": ["Limited experience with our tech stack"],
                        "hire_recommendation": "Strong Yes - excellent technical match",
                        "notable_achievements": ["Led team of 5", "Improved performance by 50%"],
        ...
        assert result["data_quality"] == "Complete profile with all relevant information"
    ...
    @patch("urllib.request.urlopen")
    def test_score_candidate_with_company_context(self, mock_urlopen, scorer, sample_candidate_profile, sample_job_description, sample_company_context):
    ...
    @patch("urllib.request.urlopen")
    def test_score_candidate_json_parse_error(self, mock_urlopen, scorer, sample_candidate_profile, sample_job_description):
    ...
    @patch("urllib.request.urlopen")
    @patch("time.sleep")
    def test_score_candidate_rate_limit_retry(self, mock_sleep, mock_urlopen, scorer, sample_candidate_profile, sample_job_description):
    ...
    @patch("urllib.request.urlopen")
    @patch("time.sleep")
    def test_score_candidate_all_retries_fail(self, mock_sleep, mock_urlopen, scorer, sample_candidate_profile, sample_job_description):
    ...
    @patch("urllib.request.urlopen")
    def test_score_candidate_http_error(self, mock_urlopen, scorer, sample_candidate_profile, sample_job_description):
        """Test handling of non-429 HTTP errors"""
        # Mock 500 error with response body
        error_500 = urllib.error.HTTPError(
            "url", 500, "Internal Server Error", {}, None
        )
        error_500.fp = Mock()
        error_500.fp.read.return_value = b"Server error details"
    ...
    @patch("urllib.request.urlopen")
    def test_prompt_construction(self, mock_urlopen, scorer, sample_candidate_profile, sample_job_description):
