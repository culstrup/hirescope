    ...
    def analyze_job(
        ...
        for i, app in enumerate(applications):
            ...
            try:
                ...
                # Process attachments
                attachments_data = self._process_candidate_attachments(
                    app, candidate_name
                )

                # Build candidate profile
                profile = self._build_candidate_profile(
                    app, candidate, attachments_data
                )
                ...
                score_result.update(
                    {
                        ...
                        "current_stage": app.get("current_stage", {}).get(
                            "name", "Unknown"
                        ),
                    }
                )
        ...
        return {
            ...
            "top_candidate": (
                sorted(results, key=lambda x: x["score"], reverse=True)[0]
                if results
                else None
            ),
        }
            ...
            for field, value in cf.items():
                if value:
                    ...
                    if isinstance(value, dict) and "value" in value:
                        desc_parts.append(
                            f"- {field_name}: ${value['value']} {value.get('unit', '')}"
                        )
                    else:
            ...
            if sample_app and sample_app[0].get("answers"):
                ...
                for answer in sample_app[0]["answers"]:
                    ...
                    if any(
                        ...
                        for term in ["experience", "years", "certification", "skill"]
                    ):
    ...
    def _process_candidate_attachments(self, app: Dict, candidate_name: str) -> Dict:
        ...
        for attachment in app.get("attachments", []):
            try:
                ...
                print(
                    f"      ⚠️  Failed to process {attachment.get('filename', 'attachment')}: {str(e)}"
                )

        return attachments_data

    def _build_candidate_profile(
        self, app: Dict, candidate: Dict, attachments_data: Dict
    ) -> str:
        """Build comprehensive candidate profile"""
        name = f"{candidate.get('first_name', '')} {candidate.get('last_name', '')}"
        email = (
            candidate.get("email_addresses", [{}])[0].get("value", "N/A")
