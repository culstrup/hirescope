...
class GreenhouseClient:
    ...
    def _make_request(self, endpoint: str, method: str = "GET") -> Dict:
        ...
        try:
            ...
            if e.code == 429:  # Rate limited
                ...
                raise Exception(f"Permission denied. Check API key permissions for: {endpoint}")
            else:
            ...
            try:
                ...
                for job in jobs:
                    try:
                        ...
                        apps = self._make_request(f"applications?job_id={job['id']}&per_page=1")
                        if apps:
                            all_jobs.append({
                                'id': job['id'],
                                'name': job['name'],
                                ...
                                'application_count': len(apps)  # This is just 1, but indicates presence
                            })
                    except:
                        continue
    ...
    def get_applications(self, job_id: int, limit: Optional[int] = None) -> List[Dict]:
        ...
        while True:
            try:
                apps = self._make_request(f"applications?job_id={job_id}&per_page=100&page={page}")
                if not apps:
                    break
