...
class ReportGeneratorV2:
    ...
    def generate_report(
        ...
        json_path = os.path.join(job_folder, "full_results.json")
        ...
        for i, candidate in enumerate(sorted_results[:5], 1):
            ...
            gh_link = self._generate_greenhouse_link(
                candidate.get("candidate_id", ""), candidate.get("application_id", "")
            )
            print(f"   🔗 {gh_link}")
...
"""
        ...
        # Save report
        filename = os.path.join(output_folder, "Full_Report.md")
    ...
    def _generate_csv_summary(
        ...
        filename = os.path.join(output_folder, "Top_Candidates.csv")
