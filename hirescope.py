...
def display_jobs_menu(jobs):
    ...
    for i, job in enumerate(jobs, 1):
        ...
        print(f"{i:<4} {status_icon} {status_text:<6} {job['name'][:40]:<40} {job['department'][:20]:<20} {job['created_at']}")
...
def get_user_selections():
    ...
    while True:
        try:
            choice = input("\n📍 Enter job number to analyze (or 'q' to quit): ").strip()
    ...
    while True:
        try:
            num_str = input("\nHow many top candidates to identify? (5-20, default=10): ").strip()
...
def confirm_analysis(job, num_top, company_context):
    ...
    print("The analysis may take several minutes depending on the number of candidates.")
...
def main():
    ...
    # Run analysis
    print("\n🚀 Starting analysis...")
    print("You can monitor progress below. The analysis will save checkpoints automatically.\n")
